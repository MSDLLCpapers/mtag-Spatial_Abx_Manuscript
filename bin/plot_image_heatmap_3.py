"""
plot_image_heatmap_3.py
    The third step in a 3 script process.

THIS SCRIPT DOES NOT REPLACE 'plot_study_heatmap_3.py'.  It cannot generate heatmap
    for an entire study, only for each of the 12 files in a study.
Run plot_study_heatmap_3.py to get a heatmap for the entire study.
...
Create a 50x50 heatmap from 50x50 matrix using Seaborn.
...
def remove_zeros_from_list(alist):  # NOT USED
    # remove all entries in 2xN list that contain only zeros
    num = len(alist)

    # how many zeros in the list?
    numZeros = 0
    for i in range(1,num):  # ignore 1st entry in list (a bugID of zero)
        if alist[i] == 0:
            numZeros += 1

    # initialize an empty 2x(num-numZeros) matrix
    outList = numpy.zeros([2, num - numZeros + 1])

    # step through list, if count > 0, add the bug ID and count to the new list
    idx = 0
    for c in range(1,numCols):
        if alist[c] > 0:
            outList[idx,0] = c
            outList[idx,1] = alist[c]
            idx += 1

    if DEBUG:
        print( 'saved ', idx, ' nonzero items in list (numZeros=', numZeros, ')')

    return outList  # a 2xN matrix
...
def create_heatmap(affinityRatiosM, radPath, studyPrefix, replicate, fov):
    ...
    nr, nc = affinityRatiosM.shape

    if 1 or DEBUG:
        print( 'Heatmap affinityRatios NumRows: ', nr, ' NumCols: ', nc)  # includes 0 row and col

    if replicate == 3 and fov == 3:
        print( 'shape affinityRatios: ', affinityRatiosM.shape)
    ...
    maxRatio = numpy.amax(affinityRatiosM[1:nr, 1:nc])  # always swill be 2.0

    colNameFloats = affinityRatiosM[0, 1:nc]  # bug IDs are in row 0 of affinity matrix
    colNameInts = [int(x) for x in colNameFloats]
    colNames = ["%3d" % x for x in colNameInts]

    df = pandas.DataFrame(affinityRatiosM[1:nr, 1:nc], index=colNames, columns = colNames)
    ...
    heatmapTitle = studyPrefix + '_radius_' + str(RADIUS_IN_MICRONS) + '_rep_' + str(replicate) + '_fov_' + str(fov)
    ...
    outName = radPath + os.sep + 'heatmap_rep_' + str(replicate) + '_fov_' + str(fov)

    # write plot to file, png and pdf
    plt.savefig(outName + '.png')
    plt.savefig(outName + '.pdf')

    # clear plot (otherwise call to sns.heatmap will plot atop old plot)
    plt.clf()
...
if SUB_FOLDER != '':
    RESULTS_PATH = RESULTS_PATH + os.sep + 'round02'
...
# for one study, loop through all of its 3 replicates and 4 fovs (12 passes)
#     create heatmaps and write the matrix CSVs for each
for replicate in [1, 2, 3]:
    for fov in [1, 2, 3, 4]:

        # the path to one file's neighborhood count matrix for each bug
        countMatrixFilename = radPath + os.sep + 'nhood_counts_rep_' + str(replicate) + '_fov_' + str(fov) + '_500.csv'
        # example filename:
        #    results / [round02 /] 3454_COLON / radius_5_nhood_counts_500.csv (without spaces)

        if not os.path.exists(countMatrixFilename):
            print( 'Skipping missing ', countMatrixFilename)
            continue
        ...
        #     nhood_search_2.py.
        nhoodCountMatrix500 = numpy.loadtxt(open(countMatrixFilename, 'r'), delimiter=',') # was rb
        ...
        bugCountFilename = studyPath + os.sep + 'bug_counts_rep_' + str(replicate) + '_fov_' + str(fov) + '_500.csv'

        if not os.path.exists(bugCountFilename):
            print( 'Skipping missing ', bugCountFilename)
            continue
        ...
        bugCounts500 = numpy.loadtxt(open(bugCountFilename, 'r'), delimiter=',') # was rb
        ...
        # write matrix without zero cols or rows to
        #     RESULTS_PATH / STUDYNAME (not into RESULTS_PATH / STUDYNAME / radius_5)
        outName = radPath + os.sep + \
        'nhood_counts_rep_' + str(replicate) + '_fov_' + str(fov) + '_no0.csv'
        with open(outName, 'w', newline='') as csvfile:
            csvWriter = csv.writer(csvfile, delimiter=',')
            # csvWriter.writerows(list(zip(*nhoodCountMatrixNo0)))
        ...
        if DEBUG:
            print('Writing bugCountsNo0', bugCountsNo0.shape)

        outName = studyPath + os.sep + 'bug_counts_rep_' + str(replicate) + '_fov_' + str(fov) + '_no0.csv'
        # with open(outName, 'w', newline='') as csvfile:
        with open(outName, 'w') as csvfile:
            # csvWriter = csv.writer(csvfile, delimiter=',')
            csvWriter = csv.writer(csvfile)
            # csvWriter.writerows(list(zip(*bugCountsNo0)))
        ...
        if DEBUG:
            print( 'Filling expectedNhoodCountMatrix ' + studyPrefix  + '_rep_' + str(replicate) + '_fov_' + str(fov))
        ...
        if CHECK_DATA:
            print( 'expectedNhoodCountMatrix shape',  expectedNhoodCountMatrix.shape)
            print( 'nhoodBugFraction shape', nhoodBugFraction.shape)
            print( 'refBugNhoodCountTotal shape', refBugNhoodCountTotal.shape)
        ...
        for r in range(1,expectedNhoodCountMatrix.shape[0]):      # refbug row
            for n in range(1,expectedNhoodCountMatrix.shape[1]):  # nhood bug column
                ...
                if expectedNhoodCountMatrix[r,n] == 0:
                    zeros += 1
                    print( zeros, ' Zero found in expectedNhoodCountMatrix at: ', r, n)

        if DEBUG:
            print( 'shape of expectedNhoodCountMatrix=', expectedNhoodCountMatrix.shape)

        # write expected nhood count array
        outName = radPath + os.sep + 'expected_nhood_count_matrix_rep_' + str(replicate) + '_fov_' + str(fov) + '_no0.csv'
        with open(outName, 'w', newline='') as csvfile:
            csvWriter = csv.writer(csvfile, delimiter=',')
        ...
        if DEBUG:
            print('Writing affinity matrix for ', studyPrefix  + '_rep_' + str(replicate) + '_fov_' + str(fov))

        outName = radPath + os.sep + 'affinity_ratio_matrix_rep_' + str(replicate) + '_fov_' + str(fov) + '_no0.csv'
        if USE_PANDAS:
            ...
            with open(outName, 'w', newline='') as csvfile:
                csvWriter = csv.writer(csvfile, delimiter=',')
                # csvWriter.writerows(list(zip(*affinityRatios)))
        ...
        # disabled for now, RLC Mar 28
        #if replicate == 3 and fov == 3:
        create_heatmap(affinityRatiosMasked, radPath, studyPrefix, replicate, fov)
